...
from torchmetrics import MeanSquaredError, MeanAbsoluteError, R2Score, MetricCollection
import warnings
import tarfile
import random
    ...
    if num_components > 1:
        ...
        for i in range(num_components):
            for j in range(i + 1, num_components):
                ...
                # Convert component sets to lists for DataFrame indexing
                comp1_list = list(component1)
                comp2_list = list(component2)
                strongest_link = sim_matrix.loc[comp1_list, comp2_list].unstack().idxmax()
        ...
        print(
            f'The weakest edge that was added for unconnected components (subgraphs) has a similarity of {min(strength_track)}.')
...
def describe_graph(adj_df):
    ...
    print("N non-zero elements in adjacency matrix:", np.count_nonzero(np.array(adj_df)), "out of",
          np.square(len(G.nodes)),
          "entries")  # make sure it should be 2* number of edges or [2*number_edges - number_nodes] if self-loops exist
...
def similarity_network(s_threshold, X_df):
    ...
    if (len(find_unconnected(np.array(adj_df))) > 0):
        print(
            f'Number of unconnected nodes: {len(find_unconnected(np.array(adj_df)))} out of {adj_df.shape[0]} nodes in G')
    ...
    if target_edges <= (num_nodes - 1):
        ...
        for _ in range(max_iter_er):
            ...
            if nx.is_connected(G):
                if abs(G.number_of_edges() - target_edges) <= int(
                        tolerance * target_edges):  # If the number of edges ~= target, convert G to adj and return adj matrix
...
def get_pos_similarity(X_df):
    ...
    # Clamp negative similarities to zero
    sim = sim.clip(lower=0)
...
def display_graph(fold, G, pos, labels_dict=None, save_fig=False, path="./", name_file="graph.png", plot_title=None,
                  wandb_log=True):
    """
...
def pad_features(x, num_heads, feat_names):
    ...
    if remainder != 0:
        ...
        pad_tensor = torch.zeros((x.size(0), pad_dim),
                                 device=x.device)  # Create a new tensor filled with zeros for padding
        x = torch.cat([x, pad_tensor],
                      dim=1)  # Concatenate the original features with the padding tensor along the feature dimension
...
def generate_model(model_name, config, data):
    ...
    Args:
        ...
        data: The data object (used to extract n_features).
    Returns:
    ...
    models_dict = {
        # Dictionary of model names and their corresponding lambda functions to instantiate the models only when they are actually needed.  This approach avoids initializing all models upfront.
        "GCNN": lambda: GCNN(
            n_features,
        ...
        "GAT": lambda: GAT(n_features, config["cl1_hidden_units"], config["cl2_hidden_units"], config["heads"],
                           config["ll_out_units"], config["dropout"]),
        "MLP2": lambda: MLP2(n_features, config["cl1_hidden_units"], config["cl2_hidden_units"], config["ll_out_units"],
                             config["dropout"]),
...
def update_overall_metrics(fold, fold_best_epoch, feat_names, relevant_features, fold_performance,
                           fold_losses, dict_val_metrics, dict_test_metrics, features_track):
    """
    ...
    dict_test_metrics["N_features"].append(len(feat_names))
    ...
    #  DEBUG: print dict_val_metrics and dict_test_metrics lengths after update
    print(f"DEBUG [fold {fold}]: dict_val_metrics lengths:")
    for key, lst in dict_val_metrics.items():
        print(f"  {key}: {len(lst)}")
    print(f"DEBUG [fold {fold}]: dict_test_metrics lengths:")
    for key, lst in dict_test_metrics.items():
        print(f"  {key}: {len(lst)}")
    return dict_val_metrics, dict_test_metrics, features_track


def cv_metrics_to_wandb(dict_val_metrics):
    """
...
def train_epoch(device, model, optimizer, criterion, data, metric):
    """Train step of model on training dataset for one epoch (regression version).
    Args:
    ...
    Returns:
        tuple: A tuple containing the training loss and the training regression metrics.
    """
    model.to(device)
    model.train()
    data.to(device)
    ...
    # Forward pass: regression output of shape [N, 1] → squeeze to [N]
    if "GAT" in model_name:  # GAT, GTC, GTC_uw, GINE, GPST_GINE_lin, GPST_GINE
        ...
        y_hat = model(x=data.x, edge_index=data.edge_index,
                      edge_weight=data.edge_attr.to(torch.float32))  # GCN, CHEBYNET, GUNet
    y_hat = y_hat.squeeze(-1)  # remove singleton feature dim for regression

    # Compute regression loss (e.g., MSE)
    ...
    # Update and compute regression metrics (MSE, MAE, R2) on training mask
    # Note: We use regression metrics instead of classification metrics.
    metric.update(y_hat[data.train_mask].cpu(), data.y[data.train_mask].cpu())
...
def evaluate_epoch(device, model, criterion, data, metric):
    """Evaluate the model on validation data for a single epoch (regression version).
    Args:
    ...
    Returns:
        tuple: A tuple containing the validation loss and the validation regression metrics.
    """
    model.eval()
    model.to(device)
    data.to(device)
    ...
    # Forward pass and squeeze as above
    model_name = str(model.__class__.__name__)
    # Forward pass: regression output of shape [N, 1] → squeeze to [N]
    if "GAT" in model_name:  # GAT, GTC, GTC_uw, GINE, GPST_GINE_lin, GPST_GINE
        y_hat = model(x=data.x, edge_index=data.edge_index, edge_attr=data.edge_attr.to(torch.float32))
        ...
        y_hat = model(x=data.x, edge_index=data.edge_index,
                      edge_weight=data.edge_attr.to(
                          torch.float32))  # GCN, CHEBYNET, GUNet    y_hat = y_hat.squeeze(-1)  # remove singleton feature dim for regression

    y_hat = y_hat.squeeze(-1)  # remove singleton feature dim for regression

    # Compute validation loss on the val_mask
    vloss = criterion(y_hat[data.val_mask], data.y[data.val_mask]).item()

    # Update and compute regression metrics on validation mask
    # Note: We use regression metrics (MSE, MAE, R2) instead of classification metrics.
    metric.update(y_hat[data.val_mask].cpu(), data.y[data.val_mask].cpu())
    val_metric = metric.compute()

    return vloss, val_metric


def test_epoch(device, model, data, metric):
    """Evaluate the model on test data for a single epoch (regression version).
    Args:
        device (torch.device): The device to perform the evaluation on.
        model (torch.nn.Module): The model to evaluate.
    ...
    Returns:
        Regression metrics on the test mask (e.g., MSE, MAE, R2).
    """
    model.eval()
    data.to(device)
    ...
    y_hat = y_hat.squeeze(-1)  # remove singleton feature dim for regression
    # Update and compute regression metrics on test mask
    # Note: We use regression metrics (MSE, MAE, R2) instead of classification metrics.
    metric.update(y_hat[data.test_mask].cpu(), data.y[data.test_mask].cpu())
...
def training(device, model, optimizer, scheduler, criterion, data, n_epochs, fold, wandb):
    ...
    wandb.define_metric(f'val/MSE-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'val/MAE-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'val/R2-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'train/MSE-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'train/MAE-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'train/R2-{fold}', step_metric=f'epoch_fold-{fold}')
    step_fold = 0
    for epoch in range(n_epochs):
        step_fold += 0
        # train
        train_loss, train_perf = train_epoch(device, model, optimizer, criterion, data,
                                             train_metrics)  # , epoch_embeddings
        # validation
...
def training_nowandb(device, model, optimizer, scheduler, criterion, data, n_epochs, fold):
    ...
    Returns:
        tuple: A tuple containing the following elements:
            - losses (list): A list of training and validation losses for each epoch.
            - perf_metrics (dict): A dictionary containing performance metrics (MSE, MAE, R2) for train and validation for each epoch.
            - best_epoch (int): The epoch number with the best validation loss.
            - best_loss (float): The best validation loss.
            - best_model (torch.nn.Module): The model with the best validation loss.
    Note:
        Returns only train and validation losses and fold performance (no test metrics).
    """
    losses = []
    # embeddings = []
    perf_metrics = {'MSE': [], 'MAE': [], 'R2': []}
...
def evaluate_mlp(device, model, criterion, data, metric):
    ...
    data = data.to(device)
    with torch.no_grad():
        y_hat = model(data.x, data.edge_index).squeeze(-1)
        val_loss = criterion(y_hat[data.val_mask], data.y[data.val_mask])
        metric.update(y_hat[data.val_mask].cpu(), data.y[data.val_mask].cpu())
        val_perf = metric.compute()
    return val_loss.item(), val_perf
...
def training_mlp(device, model, optimizer, scheduler, criterion, data, n_epochs, fold, wandb):
    ...
    wandb.define_metric(f'train/MAE-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'train/R2-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'test/MSE-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'test/MAE-{fold}', step_metric=f'epoch_fold-{fold}')
    wandb.define_metric(f'test/R2-{fold}', step_metric=f'epoch_fold-{fold}')
    for epoch in range(n_epochs):
        # train
        ...
        wandb.log({
            ...
            f'val/MSE-{fold}': val_perf['MSE'].item(),
            f'val/MAE-{fold}': val_perf['MAE'].item(),
            f'val/R2-{fold}': val_perf['R2'].item(),
            f'train/MSE-{fold}': train_perf['MSE'].item(),
            f'train/MAE-{fold}': train_perf['MAE'].item(),
            f'train/R2-{fold}': train_perf['R2'].item(),
            f'test/MSE-{fold}': test_perf['MSE'].item(),
            f'test/MAE-{fold}': test_perf['MAE'].item(),
            f'test/R2-{fold}': test_perf['R2'].item(),
        })
        if epoch % 5 == 0:
            print(f"Epoch {epoch}",
                  f"Loss train {train_loss}",
...
def feature_importance_gnnexplainer(model, data, names_list=None, save_fig=False, name_file='feature_importance',
                                    path=None, n=20):
    """
    ...
    if "MLP" in model_name:  # for unweighted models
        try:
            ...
            "GAT" in model_name or "GTC" in model_name or "GINE" in model_name) and not model_name == "GAT_uw":  # GAT, GTC, GTC_uw, GINE, GPST_GINE_lin, GPST_GINE
...
def feature_importances_shap_values(model, data, device, names_list=None, n=20, save_fig=False,
                                    name_file='feature_importance', path=None):
    """
    ...
    shap_df = pd.DataFrame(np.squeeze(shap_values), columns=names_list)
    ...
    # plot_from_shap_values(shap_values, X, save_fig=save_fig, name_file=name_file, path=path, names_list=names_list,
    #                      plot_title=None)
