...
def evaluate_config_on_fold(config, W, mastertable_file, outer_fold, param_set):
    ...
    for fold, (train_msk, val_msk) in enumerate(zip(raw_train, raw_val)):
        ...
        elif "DOS" in name.upper():
            # DOS-GNN: two-stage training
            _, performance, best_epoch, _, _, _, _, _, _, _ = training_nowandb_dos_gnn(
                check_cuda(), model, optimizer, scheduler, criterion, data, n_epochs, fold)
        else:
