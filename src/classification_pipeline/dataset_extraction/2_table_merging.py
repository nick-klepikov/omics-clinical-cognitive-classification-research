...
for fold in range(5):
    df_master = pd.read_csv(f"/Users/nickq/Documents/Pioneer Academics/Research_Project/data/data_splits/cv_folds/train_fold_{fold}_thresh_{args.threshold}.csv")
    ...
    genotype_cols = [
     ...
     # Combine genotype and clinical data
    df_geno_clin = df_master[clinical_cols + ["label"] + genotype_cols]
    df_geno_clin.to_csv(
        f"/Users/nickq/Documents/Pioneer Academics/Research_Project/data/final_datasets_unprocessed/geno_plus_clinical_fold_{fold}_thresh_{args.threshold}.csv",
        index=False
    )

     # Combine transcriptomics and clinical data
    df_rna_clin = df_master[clinical_cols + ["label"] + transcriptomics_cols]
    df_rna_clin.to_csv(
        f"/Users/nickq/Documents/Pioneer Academics/Research_Project/data/final_datasets_unprocessed/rna_plus_clinical_fold_{fold}_thresh_{args.threshold}.csv",
        index=False
    )
